...
function App() {
  ...
  return (
    <div className="bg-black h-100% h-screen pt-5 text-white">
      <button className="bg-green-800 text-white mb-3 px-5 py-2 rounded-md cursor-pointer active:scale-95" onClick={getData}>Get Data</button>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-1">
        {currentItems.map((item) => (
          <a href={item.url} target="_blank" key={item.id} className="border-1 border-white">
      ...
      {/* Pagination Buttons */}
      {data.length > 0 && (
        <div className="flex justify-center gap-4 my-5 pb-5">
          <button 
            onClick={prevPage}
